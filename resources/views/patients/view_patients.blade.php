...
<div class="page-content">
   ...
   <div class="row">
      <div class="col-12">
         <div class="card">
            <div class="card-body">
               <form class="mt-2" method="GET" action="{{ url('/patients/view') }}">
                  <div class="row">
                     ...
                     @if (Auth::user()->role != 'doctor' && Auth::user()->role != 'lab' && Auth::user()->role != 'advisor')
                     <div class="col-md-3 mb-3">
                        <div class="row align-items-center g-3">
                           <div class="col-12">
                     ...
                     <table id="tech-companies-1" class="table table-striped">
                        <thead>
                           <tr>
                              ...
                              <th class="text-end" scope="col">Advisor</th>
                              @endif
                              <th class="text-end" scope="col"></th>
                              @if(Auth::user()->role == 'doctor')
                              <th class="text-end" scope="col">Continue Treatment</th>
                              <th class="text-end" scope="col">Request New Plan</th>
                              @endif
                              @if(Auth::user()->role != 'lab')
                              <!-- <th class="text-end" scope="col">Change Package</th> -->
                              @endif
                        ...
                        <tbody>
                           ...
                           <tr class="align-middle">
                              ...
                              <td>
                                 ...
                                 @elseif($patient->status == 'Treatment Plan Completed' || $patient->status == 'Shipped')
                              ...
                              <td class="text-nowrap text-end">
                                 ...
                                 <div class="dropdown font-sans-serif btn-reveal-trigger">
                                    <a class="btn btn-danger text-600 btn-sm btn-reveal-sm transition-none"
                                       ...
                                       data-boundary="viewport" aria-haspopup="true" aria-expanded="false">Requested for Advisor</a>
                                 </div>
                                 @else
                                 N/A
                                 @endif
                              ...
                              @if(Auth::user()->role == 'doctor')
                                <td class="text-nowrap text-end">
                                    <div class="dropdown font-sans-serif btn-reveal-trigger">
                                        @if($patient->status == 'Treatment Plan Completed'  || $patient->status == 'Shipped' || $patient->status == 'Production')
                                        <form method="POST" action="{{url('/patient/treatment-plan/continue')}}"
                                        id="continue-plan-{{$loop->iteration}}">@csrf <input name="patient_id"
                                        value="{{$patient->id}}" type="hidden"></form>
                                        <a class="btn btn-link text-600 btn-sm btn-reveal-sm transition-none"
                                        href="javascript:void(0);"
                                        onclick="handleTreatment({{$loop->iteration}}, '{{ $patient->first_name.' '.$patient->last_name}}')"
                                        data-boundary="viewport" aria-haspopup="true" aria-expanded="false">Continue Treatment</a>
                                        @else
                                        N/A
                                        @endif
                                    </div>
                              ...
                              <td class="text-nowrap text-end">
                                 <div class="dropdown font-sans-serif btn-reveal-trigger">
                                    {{-- @if($patient->is_completed == 1 && date("Y-m-d H:i:s") <= date( "Y-m-d H:i:s" ,
                                    strtotime($patient->treatment_plan_duration)
                                    ) && $patient->treatment_plan_duration != null) --}}
                                    @if($patient->status == 'Treatment Plan Completed' || $patient->status == 'Shipped' || $patient->status == 'Production')
                                    <form method="POST" action="{{url('/patient/treatment-plan/request')}}"
                                       id="request-plan-{{$loop->iteration}}">@csrf <input name="patient_id"
                                       value="{{$patient->id}}" type="hidden"></form>
                                    <a class="btn btn-link text-600 btn-sm btn-reveal-sm transition-none"
                                       href="javascript:void(0);" onclick="newTreatment({{$loop->iteration}}, '{{ $patient->first_name.' '.$patient->last_name}}')"
                                       data-boundary="viewport" aria-haspopup="true" aria-expanded="false">Request Plan  {{ $patient->phase + 1 }} </a>
                                    @else
                                    N/A
                                    @endif
                                 </div>
                              ...
                              @if(Auth::user()->role != 'lab')
                              <!-- <td class="text-nowrap text-end">
                                 <a class="btn btn-sm btn-falcon-default text-danger me-2 change_plan" data-id="{{$patient->treatment_plan}}" data-current="{{$patient->pricing_package}}"
                                            href="#"><span class="fas fa-cube me-2"></span>
                                            {{$patient->pricing_package == 'AL-SECRET-CONFIDENCE' ? 'SECRET CONFIDENCE' : 'SECRET SELECT'}}</a>
                                 </td> -->
                              @endif
                              @if(Auth::user()->role == 'admin' || Auth::user()->role == 'superadmin')
                              <td class="text-nowrap text-end">
                                 <a class="btn p-0 ms-2 delete" data-id="{{$patient->id}}" data-name="{{$patient->last_name . " " . $patient->first_name}}"
...
<script>
   ...
   if (confirm('Do you really want to continue the treatment for '+name+'?')) {
       const input = prompt('Please specify which Aligners would you like to receive:');
       if (input) {
           const form = document.getElementById(`continue-plan-${loopIteration}`);
   ...
   function newTreatment(loopIteration, name) {
   if (confirm('Do you really want to order a new treatment plan?')) {
       const input = prompt('Aligners lost Track at Number:');
       if (input) {
   ...
   // function handleTreatment(loopIteration, name) {
   //     if (confirm('Do you really want to continue the treatment for ' + name + '?')) {
   //         // Create a div to hold the textarea and submit button
   ...
   //         textarea.placeholder = 'Describe why you want to continue & how many aligners you need...';
   //         textarea.rows = 4; // Set the number of rows (height)
   //         textarea.cols = 50; // Set the width of the textarea
   ...
   //         // Append the container to the body (or a modal container)
   ...
   //                 hiddenInput.type = 'hidden';
   //                 hiddenInput.name = 'comment';
   //                 hiddenInput.value = textarea.value.trim(); // Get the value from the textarea

   //                 form.appendChild(hiddenInput);
   //                 form.submit();

   //                 // Cleanup: Remove the textarea and button after submitting
