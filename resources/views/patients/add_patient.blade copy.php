...
<script>
    $(function () {
        ...
        let openImageEditor = (file, callback) => {
            ...
            var ctx; // Will be initialized after canvas is ready
            var cropper; // Will be initialized after canvas is ready

            // Add image load event handler to properly initialize canvas
            image.onload = function() {
                // Wait for image to be fully loaded and rendered
                setTimeout(function() {
                    // Initialize canvas context after canvas is properly sized
                    // --- REPLACED resizeCanvas logic to align canvas to the visible image ---
                    function resizeCanvas() {
                        // Get the actual rendered image dimensions and its position inside container
                        var imageRect = image.getBoundingClientRect();
                        var containerRect = imageContainer.getBoundingClientRect();

                        // Position canvas exactly over the image within the container
                        var left = imageRect.left - containerRect.left;
                        var top = imageRect.top - containerRect.top;
                        ...
                        // Use devicePixelRatio for sharp canvas
                        var ratio = window.devicePixelRatio || 1;
                        drawingCanvas.width = Math.round(imageRect.width * ratio);
                        drawingCanvas.height = Math.round(imageRect.height * ratio);

                        // get context and scale to device pixel ratio
                        ctx = drawingCanvas.getContext('2d');
                        ctx.setTransform(ratio, 0, 0, ratio, 0, 0);

                        // redraw grid/drawings if any
                        if (showGrid) {
                            drawGrid();
                        }
                    }
                    ...
                    // Add event listeners after canvas is properly initialized
                    ...
                    // Touch support
                    drawingCanvas.addEventListener('touchstart', function(e){ e.preventDefault(); if(e.touches && e.touches[0]) startDrawing(e.touches[0]); }, {passive:false});
                    drawingCanvas.addEventListener('touchmove', function(e){ e.preventDefault(); if(e.touches && e.touches[0]) draw(e.touches[0]); }, {passive:false});
                    drawingCanvas.addEventListener('touchend', function(e){ e.preventDefault(); if(e.changedTouches && e.changedTouches[0]) stopDrawing(e.changedTouches[0]); }, {passive:false});

                    // handle window resize
                    window.addEventListener('resize', function() {
                        // re-position and resize canvas when window/layout changes
                        resizeCanvas();
                        // redraw all stored drawings/measurements/angles at logical scale
                        redrawCanvas();
                    });

                    // Initialize CropperJS after everything is ready
                    initializeCropper();

                    // If a drawing tool is the default active (currentTool != 'none'), disable cropper so canvas receives pointer events
                    try {
                        if (typeof cropper !== 'undefined' && currentTool !== 'none') {
                            cropper.disable();
                            drawingCanvas.style.pointerEvents = 'auto';
                        } else {
                            drawingCanvas.style.pointerEvents = 'none';
                        }
                    } catch (e) { /* ignore if cropper not yet ready */ }
            ...
            var startX = 0;
            var startY = 0;
            var mmPerPixel = 0.1; // default: 1 pixel = 0.1mm (10 pixels = 1mm)

            // Angle measurement variables
            var anglePoints = []; // Array to store three points for angle measurement
            var angleStep = 0; // 0: first point, 1: second point, 2: third point

            // Storage for all measurements to persist them
            var allMeasurements = []; // Array to store all completed measurements
            var allAngles = []; // Array to store all completed angle measurements
            var allDrawings = []; // Array to store all brush strokes and drawings

            // Grid variables
            var showGrid = false;
            var gridSpacing = 50; // pixels between grid lines
            ...
            function resizeCanvas() {
                ...
                // Set canvas dimensions to match the actual image display size
                drawingCanvas.width = imageRect.width;
                drawingCanvas.height = imageRect.height;

                // Keep canvas positioned over the image container
            ...
            function startDrawing(e) {
                ...
                if (currentTool === 'angle') {
                    // Handle angle measurement (three clicks)
                    anglePoints[angleStep] = { x: pos.x, y: pos.y };
                    // Show a dot immediately where the user clicked
                    drawPoint(pos.x, pos.y);
                    angleStep++;

                    if (angleStep === 3) {
                        // All three points collected, draw the angle
                ...
                // Show start dot immediately for measure tool
                if (currentTool === 'measure') {
                    drawMeasureStartDot(pos.x, pos.y);
                }
            ...
            function draw(e) {
                ...
                if (currentTool === 'brush') {
                    ...
                    // Store the brush stroke for redrawing
                    allDrawings.push({
                        type: 'brush',
                        fromX: lastX,
                        fromY: lastY,
                    ...
                    // Store the eraser stroke for redrawing
                    allDrawings.push({
                        type: 'eraser',
                        fromX: lastX,
                        fromY: lastY,
                    ...
                    drawMeasurePreview(startX, startY, pos.x, pos.y);
                }
            ...
            function stopDrawing(e) {
                ...
                if (currentTool === 'rectangle') {
                    ...
                    drawMeasurement(startX, startY, pos.x, pos.y);
                }
            ...
            // Helper: draw a small point (used for angle clicks preview)
            ...
            function drawMeasurement(x1, y1, x2, y2) {
                // Store this measurement for persistence
                var measurement = {
                    x1: x1,
                    y1: y1,
                    x2: x2,
                ...
                drawStoredMeasurement(measurement);
            }

            function drawStoredMeasurement(measurement) {
                ctx.globalCompositeOperation = 'source-over';
                ctx.strokeStyle = measurement.color;
                ctx.lineWidth = measurement.lineWidth;
                ctx.globalAlpha = measurement.opacity;

                // Calculate distance in pixels
                var distancePx = Math.sqrt(Math.pow(measurement.x2 - measurement.x1, 2) + Math.pow(measurement.y2 - measurement.y1, 2));
                var labelText;
                if (mmPerPixel && mmPerPixel > 0) {
                    var distanceMm = distancePx * mmPerPixel;
                    labelText = distanceMm.toFixed(2) + ' mm';
                } else {
                    labelText = distancePx.toFixed(1) + ' px';
                }
                ...
                // Draw prominent start and end point dots
                var dotRadius = 6; // Larger radius for better visibility
                var dotStrokeWidth = 2;

                // Start point dot
                ctx.beginPath();
                ctx.arc(measurement.x1, measurement.y1, dotRadius, 0, 2 * Math.PI);
                ctx.fillStyle = 'white'; // White fill
                ctx.fill();
                ctx.strokeStyle = measurement.color; // Colored border
                ctx.lineWidth = dotStrokeWidth;
                ctx.stroke();

                // End point dot
                ctx.beginPath();
                ctx.arc(measurement.x2, measurement.y2, dotRadius, 0, 2 * Math.PI);
                ctx.fillStyle = 'white'; // White fill
                ctx.fill();
                ctx.strokeStyle = measurement.color; // Colored border
                ctx.lineWidth = dotStrokeWidth;
                ...
                // Background for text sized to text length
                ctx.font = '20px Arial';
                var textWidth = ctx.measureText(labelText).width;
                var padX = 10, padY = 8;
                ctx.fillStyle = 'rgba(255, 255, 255, 0.85)';
                ctx.fillRect(midX - textWidth/2 - padX, midY - 14, textWidth + padX*2, 28);
            ...
            // Helper function to draw start dot for measure tool
            function drawMeasureStartDot(x, y) {
                var dotRadius = 6;
                var dotStrokeWidth = 2;
            ...
            // Helper function to draw preview line during measure tool drawing
            function drawMeasurePreview(x1, y1, x2, y2) {
                // Draw preview line
                ctx.globalCompositeOperation = 'source-over';
                ctx.strokeStyle = currentColor;
                ctx.lineWidth = currentSize;
                ctx.globalAlpha = currentOpacity * 0.7; // Slightly transparent for preview
                ...
                // Draw start dot
                drawMeasureStartDot(x1, y1);

                // Draw end dot preview
                var dotRadius = 6;
                var dotStrokeWidth = 2;
                ctx.beginPath();
            ...
            function drawAngle(p1, p2, p3) {
                // Store this angle for persistence
                var angleData = {
                    p1: { x: p1.x, y: p1.y },
                    p2: { x: p2.x, y: p2.y },
                    p3: { x: p3.x, y: p3.y },
                ...
                allAngles.push(angleData);

                // Draw the angle
                drawStoredAngle(angleData);
            }

            function drawStoredAngle(angleData) {
                ctx.globalCompositeOperation = 'source-over';
                ctx.strokeStyle = angleData.color;
                ctx.lineWidth = angleData.lineWidth;
                ctx.globalAlpha = angleData.opacity;

                // Calculate angle between three points (p2 is the vertex)
                var angle = calculateAngle(angleData.p1, angleData.p2, angleData.p3);
                var angleDegrees = angle * (180 / Math.PI);

                // Draw the two lines forming the angle
                ctx.beginPath();
                ctx.moveTo(angleData.p1.x, angleData.p1.y);
                ctx.lineTo(angleData.p2.x, angleData.p2.y);
                ctx.lineTo(angleData.p3.x, angleData.p3.y);
                ctx.stroke();

                // Draw points
                ctx.beginPath();
                ctx.arc(angleData.p1.x, angleData.p1.y, 3, 0, 2 * Math.PI);
                ctx.fill();
                ctx.beginPath();
                ctx.arc(angleData.p2.x, angleData.p2.y, 4, 0, 2 * Math.PI);
                ctx.fill();
                ctx.beginPath();
                ctx.arc(angleData.p3.x, angleData.p3.y, 3, 0, 2 * Math.PI);
                ctx.fill();

                // Draw angle arc
                var radius = 30;
                var startAngle = Math.atan2(angleData.p1.y - angleData.p2.y, angleData.p1.x - angleData.p2.x);
                var endAngle = Math.atan2(angleData.p3.y - angleData.p2.y, angleData.p3.x - angleData.p2.x);

                ctx.beginPath();
                ctx.arc(angleData.p2.x, angleData.p2.y, radius, startAngle, endAngle);
                ctx.stroke();

                // Draw angle label
                var labelText = angleDegrees.toFixed(1) + '°';
                var labelX = angleData.p2.x + Math.cos((startAngle + endAngle) / 2) * (radius + 20);
                var labelY = angleData.p2.y + Math.sin((startAngle + endAngle) / 2) * (radius + 20);
                ...
                var padX = 10, padY = 8;
                ctx.fillStyle = 'rgba(255, 255, 255, 0.85)';
                ctx.fillRect(labelX - textWidth/2 - padX, labelY - 14, textWidth + padX*2, 28);

                // Text
                ctx.fillStyle = angleData.color;
                ctx.textAlign = 'center';
                ctx.textBaseline = 'middle';
            ...
            function clearDrawing() {
                ...
                // Clear all stored measurements, angles, and drawings
                allMeasurements = [];
                allAngles = [];
                allDrawings = [];
            ...
            function redrawCanvas() {
                ...
                // Redraw all stored brush strokes and eraser marks
                allDrawings.forEach(function(drawing) {
                    ctx.save();
                ...
                // Redraw all stored angles
                allAngles.forEach(function(angleData) {
                    drawStoredAngle(angleData);
                });
            }
            ...
            function drawGrid() {
                ...
                // Draw vertical lines
                for (var x = 0; x <= drawingCanvas.width; x += gridSpacing) {
                    ctx.beginPath();
                    ctx.moveTo(x, 0);
                ...
                for (var y = 0; y <= drawingCanvas.height; y += gridSpacing) {
                    ctx.beginPath();
                    ctx.moveTo(0, y);
            ...
            // Canvas initialization is now handled in image.onload

            // --- REMOVED duplicate resizeCanvas() definition that set left/top to 0 and width/height:100% ---
            /* removed:
            function resizeCanvas() {
                // Get the actual rendered image dimensions
                var imageRect = image.getBoundingClientRect();
            ...
            */
            ...
            ratios.forEach(function(ratio) {
                var btn = createButton(ratio.text, 'btn-outline-light btn-sm', function() {
                    if (cropper) cropper.setAspectRatio(ratio.value);
                });
                btn.style.cssText = `
                    padding: 8px 12px;
                    font-size: 12px;
            ...
            rotationPresets.forEach(function(preset) {
                var btn = createButton(preset.text, 'btn-outline-light btn-sm', function() {
                    if (cropper) cropper.rotate(preset.value);
                });
                btn.style.cssText = `
                    padding: 8px 12px;
                    font-size: 12px;
            ...
            moveButtons.forEach(function(btn) {
                var button = createButton(btn.text, 'btn-outline-light btn-sm', function() {
                    if (btn.text === '⌂') {
                        if (cropper) cropper.center();
                    } else {
                        if (cropper) cropper.move(btn.dir[0], btn.dir[1]);
                    }
                });
            ...
            var flipHBtn = createButton('Flip H', 'btn-outline-light btn-sm', function() {
                if (cropper) cropper.scaleX(-cropper.getImageData().scaleX);
            });
            ...
            var flipVBtn = createButton('Flip V', 'btn-outline-light btn-sm', function() {
                if (cropper) cropper.scaleY(-cropper.getImageData().scaleY);
            });
            ...
            var undoBtn = createButton('Undo', 'btn-outline-info btn-sm', function() {
                // Simple undo - clear and redraw (for now)
                clearDrawing();
            });
            ...
            gridTitle.textContent = '📐 Grid Lines';
            gridTitle.style.cssText = 'font-weight:600;margin-bottom:8px;';

            var gridToggleRow = document.createElement('div');
            gridToggleRow.style.cssText = 'display:flex;gap:8px;align-items:center;margin-bottom:8px;';
            var gridToggle = document.createElement('input');
            gridToggle.type = 'checkbox';
            gridToggle.id = 'gridToggle';
            gridToggle.addEventListener('change', function() {
                showGrid = this.checked;
                // Redraw canvas preserving all drawings
                redrawCanvas();
            });
            var gridLabel = document.createElement('label');
            gridLabel.htmlFor = 'gridToggle';
            gridLabel.textContent = 'Show Grid';
            gridLabel.style.cssText = 'color:#fff;cursor:pointer;';
            gridToggleRow.appendChild(gridToggle);
            gridToggleRow.appendChild(gridLabel);

            var gridSpacingRow = document.createElement('div');
            gridSpacingRow.style.cssText = 'display:flex;gap:8px;align-items:center;';
            var spacingLabel = document.createElement('label');
            spacingLabel.textContent = 'Spacing:';
            spacingLabel.style.cssText = 'color:#fff;min-width:60px;';
            var spacingInput = document.createElement('input');
            spacingInput.type = 'number';
            spacingInput.value = gridSpacing;
            spacingInput.min = '10';
            spacingInput.max = '200';
            spacingInput.style.cssText = 'width:80px;padding:6px;border-radius:4px;border:1px solid #444;background:#222;color:#fff;';
            spacingInput.addEventListener('change', function() {
                gridSpacing = parseInt(this.value) || 50;
                if (showGrid) {
                    // Redraw canvas preserving all drawings
                    redrawCanvas();
                }
            });
            var spacingUnit = document.createElement('span');
            spacingUnit.textContent = 'px';
            spacingUnit.style.cssText = 'color:#fff;';

            gridSpacingRow.appendChild(spacingLabel);
            gridSpacingRow.appendChild(spacingInput);
            gridSpacingRow.appendChild(spacingUnit);

            gridControls.appendChild(gridTitle);
            gridControls.appendChild(gridToggleRow);
            gridControls.appendChild(gridSpacingRow);
            sidebar.appendChild(gridControls);
