        ...
        @else
            ...
            @if (($patient->is_completed == 0 || Auth::user()->role == 'staff' || Auth::user()->role == 'lab') && ($patient->is_submitted == 0 || $patient->is_editable == 0))
                ...
                @if (Auth::user()->role == 'staff')
                    @include('patients.case-overview.commentbox.staff')
                @elseif (Auth::user()->role == 'lab')
                    @include('patients.case-overview.commentbox.lab')
                @elseif (Auth::user()->role == 'doctor')
                    @include('patients.case-overview.commentbox.doctor')
                @endif
            ...
            <div class="card">
                <div class="card-body">
                    @if (Auth::user()->role == 'doctor' && $patient->is_editable == 0 && $patient->is_submitted != 0)
                        @if (($patient->recommended_advisor == null && $patient->is_treatment_submitted == 0) ||
                                ($patient->recommended_advisor == null && $patient->is_treatment_submitted == 1))
                            <label><strong>Send to Advisor</strong></label>
                            <hr>
                            <form id="sendToAdvisor" method="POST"
                                action="{{ url('/patient/case-overview/send-from-doctor-to-staff-for-advisor') }}">
                                @csrf
                                <input type="hidden" name="treatment_plan_id"
                                    value="{{ $patient->id }}" />
                                <div class="row">
