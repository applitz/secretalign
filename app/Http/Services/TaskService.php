...
class TaskService
{
    ...
    // create_task_withoutMailStaff Starts
    public function create_task_withoutMailStaff($type, $task, $user_id = null, $comment = null, $from_role = null, $to_role = null,$attachments=null)
    {
        if (!DB::table("tasks")->where("treatment_plan_id", $this->treatment_plan_id)->where("type", $type)->where("status", "pending")->exists()) {
           //dd("true");
            $latest = DB::table('tasks')->insertGetId([
            ...
            $this->liveAlertStaff("You have a new task.", $user_id, $type, $latest);
                        return $latest;
        }
    }
    // create_task_withoutMailStaff Ends

    // liveAlertStaff Starts
    public function liveAlertStaff($body, $user_id, $type, $task_id = null)
    {
        $path = storage_path('al-secret-dev-firebase-adminsdk-fe376-23fb4976a1.json');
        $data = ['role' => $type,    'user' => $user_id,];
        if ($type == 'doctor' || $type == 'lab' || $type == 'superadmin' || $type == 'advisor') {
        ...
        if ($type == 'staff') {
            foreach (DB::table('users')->where('id', $user_id)->pluck('staff_id')->toArray() as $u) {
                DB::table('notifications')->insert([
                    "type" => $type,
                    "body" => $body,
                    "task_id" => $task_id,
